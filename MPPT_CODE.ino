...
void setup(){
  ...
  digitalWrite(load_enable,HIGH);      //Start with the relay turned off           /////////////////////// LOW
...
void loop(){
  ...
  //  Serial.println(bat_voltage);
   ...
   // Serial.println(solar_current);
    ...
    str =String("Battery: ")+String(bat_voltage)+String(" V ," )+String("Solar: ")+String(solar_voltage )+String(" V ," )+String("solar current: ")+String(solar_current)+String(" A ," )+String("Power: ")+String(solar_power)+String(" W");
    Serial.println(str); // to send data into node mcu 
  }
  
  if(bat_voltage < battery_min_voltage){
    digitalWrite(load_enable,HIGH);        //We DISABLE the load if battery is undervoltage  ///////////////LOW
    load_status  = "OFF";
  }
  else{
    digitalWrite(load_enable,LOW);       //We ENABLE the load if battery charged  ////////////////////////HIGH
    load_status  = "ON";
  }
...
float get_solar_voltage(int n_samples)
{
  ...
  {
    voltage += (analogRead(solar_voltage_in) * (5.0 / 1023.0) * 5.0); 
  }
...
float get_battery_voltage(int n_samples)
{
  ...
  for(int i=0; i < n_samples; i++)
  {
    voltage += (analogRead(battery_voltage_in) * (5.0 / 1023.0) * 5.0);     
  }
  voltage = voltage/n_samples;
  if(voltage < 0){voltage = 0;}
  return(voltage);
